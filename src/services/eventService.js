    ...
    async initStatements() {
        ...
        this.statements = {
            insertEvent: await this.db.prepare(`
                INSERT INTO events (name, description, event_type, server_name, event_time, timezone, character_id, participation_status, location, recurring_pattern, notification_enabled, notification_minutes, war_type)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `),
            updateEvent: await this.db.prepare(`
                UPDATE events 
                SET name = ?, description = ?, event_type = ?, server_name = ?, event_time = ?, timezone = ?, character_id = ?, participation_status = ?, location = ?, recurring_pattern = ?, notification_enabled = ?, notification_minutes = ?, war_type = ?
                WHERE id = ?
            `),
