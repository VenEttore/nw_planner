...
<span class={`inline-flex items-center rounded border ${bgClass} ${textClass}`}>
  <select
    class={`appearance-none bg-transparent border-0 focus:ring-0 outline-none ${selectClass} pr-6 pl-2 py-1 bg-[length:12px_12px] bg-no-repeat bg-right-2 bg-[url('data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'16\' height=\'16\' viewBox=\'0 0 20 20\' fill=\'none\' stroke=\'%23666\' stroke-width=\'2\' stroke-linecap=\'round\' stroke-linejoin=\'round\'><polyline points=\'6 8 10 12 14 8\'/></svg>')]`}
    bind:value
    disabled={disabled}
    on:click={maybeStop}
